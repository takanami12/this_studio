...
1. Download/load raw phage genome data (DeePhage + DeepPL)
...
from utils.data_download import download_deephage_data, download_deeppl_data, prepare_genome_dataset
...
def main():
    ...
    # ================================================================
    # Step 1: Download/Load raw genome data
    # ================================================================
    print("=" * 60)
    ...
    if len(genomes) == 0:
        print("\nNo genomes loaded. Please place your FASTA files in the data directory.")
        print("Expected structure:")
        print(f"  {args.data_dir}/virulent.fastvirulent phage genomes)")
        print(f"  {args.data_dir}/temperate.fasta  (707 temperate phage genomes)")
        print("\nAlternatively, place them in:")
        print(f"  {args.data_dir}/deephage/data/virulent.fasta")
        print(f"  {args.data_dir}/deephage/data/temperate.fasta")
        print("\nYou can also provide a single combined FASTA with labels in headers.")
        return
    ...
    for fold_idx, (train_val_idx, test_idx) in enumerate(skf.split(sequences, labels)):
        ...
        n_train = int(len(train_val_seqs) * args.train_ratio)
    ...
    for group_name, group_config in CONTIG_GROUP_CONFIGS.items():
        ...
        for fold_idx, fold_data in enumerate(folds):
            ...
            for split_name in ['train', 'val', 'test']:
                ...
                # Generate contigs (with reverse complement for train only, 
                # or for all as per paper)
